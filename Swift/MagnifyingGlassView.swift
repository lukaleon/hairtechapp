...
public class MagnifyingGlassView: UIView {
    ...
    ///
   @objc public weak var magnifiedView: UIView? = nil {
        didSet {
            removeFromSuperview()
   ...
   @objc public var magnifiedPoint: CGPoint = CGPoint.zero {
        didSet {
    ...
    /// Offset by which magnification glass will be shifted from `magnifiedPoint`
     @objc public var offset: CGPoint = CGPoint.zero

    /// Radius of magnification glass view
    @objc public var radius: CGFloat = 50.0 {
        didSet {
    ...
    @objc public var scale: CGFloat = 2.0
    
    /// Border color of magnification glass
    @objc public var borderColor: UIColor = UIColor.lightGray {
        didSet {
    ...
    @objc  public var borderWidth: CGFloat = 3.0 {
        didSet {
    ...
    @objc  public var showsCrosshair: Bool = true {
        didSet {
    ...
    /// Crosshair color
    @objc  public var crosshairColor: UIColor = UIColor.lightGray {
        didSet {
    ...
    /// Crosshair width
    @objc  public var crosshairWidth: CGFloat = 0.5 {
        didSet {
    ...
    @objc   private let crosshair: CAShapeLayer = CAShapeLayer()
    ...
    ///
    @objc public convenience init(offset: CGPoint = CGPoint.zero,
                            radius: CGFloat = 50.0,
                            scale: CGFloat = 2.0,
    ...
    ///
  @objc  public func magnify(at point: CGPoint) {
        guard magnifiedView != nil else { return }

        magnifiedPoint = point
        //magnifiedPoint = CGPoint(x: point.x, y: point.y + radius)
